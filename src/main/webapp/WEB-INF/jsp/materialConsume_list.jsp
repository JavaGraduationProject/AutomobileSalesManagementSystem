...
<table class="easyui-datagrid" id="materialConsumeList" title="汽车进货管理列表" data-options="singleSelect:false,
    ...
    <thead>
        <tr>
        	...
        	<th data-options="field:'consumeId',align:'center',width:100">汽车进货管理编号</th>
            <th data-options="field:'work',align:'center',width:100,formatter:formatWork">所属车辆</th>
            <th data-options="field:'material',align:'center',width:100,formatter:formatMaterial">业务</th>
...
<div  id="toolbar_materialConsume" style=" height: 22px; padding: 3px 11px; background: #fafafa;">  
    ...
    <div id="search_materialConsume" style="float: right;">
        ...
        <div id="menu_materialConsume" style="width:120px"> 
			<div data-options="name:'consumeId'">汽车进货管理编号</div> 
			<div data-options="name:'workId'">编号</div>
			<div data-options="name:'materialId'">业务编号</div> 
		</div>     
    </div>  
...
<!-- 业务对话框内容信息 -->
<div id="materialConsumeMaterialInfo" class="easyui-dialog" title="车辆销售管理" data-options="modal:true,closed:true,
...
<!-- 工作表对话框信息内容 -->
<div id="materialConsumeWorkInfo" class="easyui-dialog" title="车辆信息" data-options="modal:true,closed:true,
...
<script>
	...
	if(value == null || value == ''){
		$("#materialConsumeList").datagrid({
	        title:'汽车进货管理列表', singleSelect:false, collapsible:true, pagination:true, rownumbers:true, method:'get',
			nowrap:true, toolbar:"toolbar_materialConsume", url:'materialConsume/list', method:'get',
			loadMsg:'数据加载中......',  fitColumns:true,//允许表格自动缩放,以适应父容器
	        columns : [ [ 	      	        
				{field : 'ck', checkbox:true },
				{field : 'consumeId', width : 100, title : '汽车进货管理编号', align:'center'},
				{field : 'work', width : 100, align : 'center', title : '所属车辆', formatter:formatWork},
				{field : 'material', width : 100, align : 'center', title : '业务', formatter:formatMaterial},
    		...
    		if(data.msg != null){
    			...
    			$.post("materialConsume/update_note",$("#materialConsumeNoteForm").serialize(), function(data){
    				if(data.status == 200){
    					...
    					$.messager.alert("操作提示", "更新汽车进货管理详情成功！");
    				}else{
    					$.messager.alert('提示',data.msg);
    				}  
    			});
