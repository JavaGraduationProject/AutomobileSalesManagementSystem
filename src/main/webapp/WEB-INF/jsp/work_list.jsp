...
<table class="easyui-datagrid" id="workList" title="车辆列表" data-options="singleSelect:false,collapsible:true,
		pagination:true,rownumbers:true,url:'work/list',method:'get',pageSize:10, fitColumns:true,toolbar:toolbar_work">
    <thead>
        <tr>
        	<th data-options="field:'ck',checkbox:true"></th>
        	<th data-options="field:'workId',align:'center',width:100">编号</th>
            <th data-options="field:'processNumber',align:'center',width:100">车辆号</th>
            <th data-options="field:'product',align:'center',width:100,formatter:formatWorkProduct">产品</th>
            <th data-options="field:'process',align:'center',width:100,formatter:formatWorkProcess">车辆</th>
            <th data-options="field:'device',align:'center',width:100,formatter:formatWorkDevice">操作人</th>
            <th data-options="field:'rating',width:100,align:'center'">备注</th>
        </tr>
    </thead>
</table>
...
<div  id="toolbar_work" style=" height: 22px; padding: 3px 11px; background: #fafafa;">  
    ...
    <div id="search_work" style="float: right;">
        ...
        <div id="menu_work" style="width:120px"> 
			<div data-options="name:'workId'">编号</div> 
			<div data-options="name:'workProduct'">产品名称</div> 
			<div data-options="name:'workDevice'">设备名称</div> 
			<div data-options="name:'workProcess'">车辆</div> 
		</div>     
    </div>  
</div>  

<div id="workEditWindow" class="easyui-window" title="编辑车辆" data-options="modal:true,closed:true,resizable:true,
	iconCls:'icon-save',href:'work/edit'" style="width:40%;height:55%;padding:10px;">
</div>
<div id="workAddWindow" class="easyui-window" title="添加车辆" data-options="modal:true,closed:true,resizable:true,
...
<div id="workProcessInfo" class="easyui-dialog" title="车辆信息" data-options="modal:true,closed:true,resizable:true,
...
function doSearch_work(value,name){ //用户输入用户名,点击搜素,触发此函数  
	if(value == null || value == ''){
		$("#workList").datagrid({
	        title:'车辆列表', singleSelect:false, collapsible:true, pagination:true, rownumbers:true, method:'get',
			nowrap:true, toolbar:"toolbar_work", url:'work/list', method:'get', loadMsg:'数据加载中......',
			fitColumns:true,//允许表格自动缩放,以适应父容器
	        columns : [ [ 
				{field : 'ck', checkbox:true },
				{field : 'workId', width : 100, title : '编号', align:'center'},
				{field : 'processNumber', width : 100, align : 'center', title : '车辆号'},
				{field : 'product', width : 100, align : 'center', title : '产品', formatter:formatWorkProduct},
				{field : 'process', width : 100, title : '车辆', align:'center', formatter:formatWorkProcess},
				{field : 'device', width : 100, title : '操作人', align:'center',formatter:formatWorkDevice},
				{field : 'rating', width : 100, title : '备注', align:'center'}
	        ] ],
	    });
	}else{
		$("#workList").datagrid({  
    ...
    function work_edit(){
    	$.get("work/edit_judge",'',function(data){
       		if(data.msg != null){
            	...
            	if(ids.length == 0){
            		$.messager.alert('提示','必须选择一个车辆才能编辑!');
            		return ;
            	}
            	if(ids.indexOf(',') > 0){
            		$.messager.alert('提示','只能选择一个车辆!');
            		return ;
            	}
