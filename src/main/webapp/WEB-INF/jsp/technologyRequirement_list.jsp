...
<table class="easyui-datagrid" id="technologyRequirementList" title="提醒用户定期保养列表" 
...
<div  id="toolbar_technologyRequirement" style=" height: 22px; padding: 3px 11px; background: #fafafa;">  
    ...
    <div id="search_technologyRequirement" style="float: right;">
        ...
        <div id="menu_technologyRequirement" style="width:120px"> 
			<div data-options="name:'technologyRequirementId'">提醒用户定期保养编号</div> 
...
<div id="technologyRequirementEditWindow" class="easyui-window" title="编辑提醒用户定期保养"
...
<div id="technologyRequirementAddWindow" class="easyui-window" title="添加提醒用户定期保养"
	 data-options="modal:true,closed:true,resizable:true,iconCls:'icon-save',href:'technologyRequirement/add'"
	 style="width:65%;height:65%;padding:10px;">
</div>

<!-- 生产人信息 -->
<div id="technologyInfo_technologyRequirement" class="easyui-dialog" title="生产人信息"
...
<div id="technologyRequirementNoteDialog" class="easyui-dialog" title="提醒用户定期保养" data-options="modal:true,closed:true,
...
function doSearch_technologyRequirement(value,name){ //用户输入用户名,点击搜素,触发此函数  
	if(value == null || value == ''){
		...
		$("#technologyRequirementList").datagrid({
	        title:'提醒用户定期保养列表', singleSelect:false, collapsible:true, pagination:true, rownumbers:true, method:'get',
			nowrap:true, toolbar:"toolbar_technologyRequirement", url:'technologyRequirement/list', method:'get',
			loadMsg:'数据加载中......', fitColumns:true,//允许表格自动缩放,以适应父容器
	        columns : [ [
				{field : 'ck', checkbox:true },
				{field : 'technologyRequirementId', width : 100, align:'center', title : '提醒用户定期保养编号'},
				{field : 'technologyName', width : 120, align : 'center', title : '生产人名称',
					formatter:formatTechnology_technologyRequirement},
				{field : 'requirement', width : 100, align : 'center', title : '提醒用户定期保养',
					formatter:formatRequirement},
				{field : 'addTime', width : 130, title : '提醒用户定期保养增加时间', align:'center',
					formatter:TAOTAO.formatDateTime},
				{field : 'reviseTime', width : 130, title : '提醒用户定期保养修改时间', align:'center',
					formatter:TAOTAO.formatDateTime}
	        ] ],  
	    });
	}else{
	...
	function formatRequirement(value, row, index){ 
		if(value !=null && value != ''){
			return "<a href=javascript:openTechnologyRequirementNote("+index+")>"+"提醒用户定期保养"+"</a>";
		}else{
			return "无";
		}
	}
		...
		$.get("technology/edit_judge",'',function(data){
    		if(data.msg != null){
    			...
    			$.post("technology/update_all",$("#technologyEditForm_technologyRequirement").serialize(),
						function(data){
    				if(data.status == 200){
    					$.messager.alert('提示','修改生产人成功!','info',function(){
    		...
    		if(data.msg != null){
    			...
    			$.post("technologyRequirement/update_requirement",$("#technologyRequirementNoteForm").serialize(),
    				...
    				if(data.status == 200){
    					...
    					$.messager.alert("操作提示", "更新提醒用户定期保养成功！");
    				}else{
    					$.messager.alert('提示',data.msg);
    				}
    			});
    ...
    function technologyRequirement_edit(){
    	$.get("technologyRequirement/edit_judge",'',function(data){
      		if(data.msg != null){
      	    	...
      	    	if(ids.length == 0){
      	    		$.messager.alert('提示','必须选择一个提醒用户定期保养才能编辑!');
      	    		return ;
      	    	}
      	    	if(ids.indexOf(',') > 0){
      	    		$.messager.alert('提示','只能选择一个提醒用户定期保养!');
      	    		return ;
      	    	}
    ...
    function technologyRequirement_delete(){
    	$.get("technologyRequirement/delete_judge",'',function(data){
       		if(data.msg != null){
       			...
       			var ids = getTechnologyRequirementSelectionsIds();
       	    	if(ids.length == 0){
       	    		$.messager.alert('提示','未选中提醒用户定期保养!');
       	    		return ;
       	    	}
       	    	$.messager.confirm('确认','确定删除ID为 '+ids+' 的提醒用户定期保养吗？',function(r){
       	    	    if (r){
       	    	    	var params = {"ids":ids};
       	            	$.post("technologyRequirement/delete_batch",params, function(data){
       	        			if(data.status == 200){
       	        				$.messager.alert('提示','删除提醒用户定期保养成功!',undefined,function(){
       	        					$("#technologyRequirementList").datagrid("reload");
       	        				});
       	        			}
       	        		});
