...
<table class="easyui-datagrid" id="materialReceiveList" title="业务收入列表" data-options="singleSelect:false,
    ...
    <thead>
         <tr>
        	...
        	<th data-options="field:'receiveId',align:'center',width:100">业务收入编号</th>
            <th data-options="field:'material',width:100,align:'center',formatter:formatMaterial">业务</th>
...
<div  id="toolbar_materialReceive" style=" height: 22px; padding: 3px 11px; background: #fafafa;">  
    ...
    <div id="search_materialReceive" style="float: right;">
        ...
        <div id="menu_materialReceive" style="width:120px"> 
			<div data-options="name:'receiveId'">业务收入编号</div> 
...
<div id="materialReceiveEditWindow" class="easyui-window" title="编辑业务收入" data-options="modal:true,closed:true,
	resizable:true,iconCls:'icon-save',href:'materialReceive/edit'" style="width:65%;height:75%;padding:10px;">
</div>
<div id="materialReceiveAddWindow" class="easyui-window" title="添加业务收入" data-options="modal:true,closed:true,
...
<div id="materialInfo" class="easyui-dialog" title="车辆销售管理" data-options="modal:true,closed:true,resizable:true,
...
function doSearch_materialReceive(value,name){ //用户输入用户名,点击搜素,触发此函数  
	if(value == null || value == ''){
		$("#materialReceiveList").datagrid({
	        title:'业务收入列表', singleSelect:false, collapsible:true, pagination:true, rownumbers:true, method:'get',
			nowrap:true, toolbar:"toolbar_materialReceive", url:'materialReceive/list', method:'get',
			loadMsg:'数据加载中......',  fitColumns:true,//允许表格自动缩放,以适应父容器
	        columns : [ [ 	       
				{field : 'ck', checkbox:true },
				{field : 'receiveId', width : 100, title : '业务收入编号', align:'center'},
    		...
    		if(data.msg != null){
    			...
    			$.post("materialReceive/update_note",$("#materialReceiveNoteForm").serialize(), function(data){
    				if(data.status == 200){
    					...
    					$.messager.alert("操作提示", "更新业务收入详情成功！");
    				}else{
    					$.messager.alert('提示',data.msg);
    				} 
    			});
