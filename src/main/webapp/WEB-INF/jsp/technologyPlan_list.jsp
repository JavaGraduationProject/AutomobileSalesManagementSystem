...
<table class="easyui-datagrid" id="technologyPlanList" title="提醒用户定期安检列表" data-options="singleSelect:false,
...
<div  id="toolbar_technologyPlan" style=" height: 22px; padding: 3px 11px; background: #fafafa;">  
    ...
    <div id="search_technologyPlan" style="float: right;">
        ...
        <div id="menu_technologyPlan" style="width:120px"> 
			<div data-options="name:'technologyPlanId'">提醒用户定期安检编号</div> 
...
<div id="technologyPlanEditWindow" class="easyui-window" title="编辑提醒用户定期安检" data-options="modal:true,closed:true,
	resizable:true,iconCls:'icon-save',href:'technologyPlan/edit'" style="width:40%;height:55%;padding:10px;">
</div>
<div id="technologyPlanAddWindow" class="easyui-window" title="添加提醒用户定期安检" data-options="modal:true,closed:true,
	resizable:true,iconCls:'icon-save',href:'technologyPlan/add'" style="width:40%;height:55%;padding:10px;">
</div>
 
<!-- 生产人信息 -->
<div id="technologyInfo_technologyPlan" class="easyui-dialog" title="生产人信息" data-options="modal:true,closed:true,
...
function doSearch_technologyPlan(value,name){ //用户输入用户名,点击搜素,触发此函数  
	if(value == null || value == ''){
		...
		$("#technologyPlanList").datagrid({
	        title:'提醒用户定期安检列表', singleSelect:false, collapsible:true, pagination:true, rownumbers:true, method:'get',
			nowrap:true, toolbar:"toolbar_technologyPlan", url:'technologyPlan/list', method:'get',
			loadMsg:'数据加载中......', fitColumns:true,//允许表格自动缩放,以适应父容器
	        columns : [ [
				{field : 'ck', checkbox:true },
				{field : 'technologyPlanId', width : 100, align:'center', title : '提醒用户定期安检编号'},
				...
				{field : 'commitPlan', width : 130, title : '计划提交时间', align:'center',
					formatter:TAOTAO.formatDateTime},
				{field : 'technologyPlanStart', width : 130, title : '提醒用户定期安检开始时间', align:'center',
					formatter:TAOTAO.formatDateTime},
				{field : 'technologyPlanEnd', width : 130, title : '提醒用户定期安检结束时间', align:'center',
					formatter:TAOTAO.formatDateTime},
	        ] ],  
	    });
	}else{
    ...
    function technologyPlan_edit(){
    	$.get("technologyPlan/edit_judge",'',function(data){
       		if(data.msg != null){
       	    	...
       	    	if(ids.length == 0){
       	    		$.messager.alert('提示','必须选择一个提醒用户定期安检才能编辑!');
       	    		return ;
       	    	}
       	    	if(ids.indexOf(',') > 0){
       	    		$.messager.alert('提示','只能选择一个提醒用户定期安检!');
       	    		return ;
       	    	}
    ...
    function technologyPlan_delete(){
    	$.get("technologyPlan/delete_judge",'',function(data){
       		if(data.msg != null){
       			...
       			var ids = getTechnologyPlanSelectionsIds();
       	    	if(ids.length == 0){
       	    		$.messager.alert('提示','未选中提醒用户定期安检!');
       	    		return ;
       	    	}
       	    	$.messager.confirm('确认','确定删除ID为 '+ids+' 的提醒用户定期安检吗？',function(r){
       	    	    if (r){
       	    	    	var params = {"ids":ids};
       	            	$.post("technologyPlan/delete_batch",params, function(data){
       	        			if(data.status == 200){
       	        				$.messager.alert('提示','删除提醒用户定期安检成功!',undefined,function(){
