...
<table  id="pCountCheckList" title="热卖汽车类型" class="easyui-datagrid" data-options="singleSelect:false,
    ...
    <thead>
        <tr>
        	...
        	<th data-options="field:'pCountCheckId',align:'center',width:100">热卖汽车类型编号</th>
...
<div  id="toolbar_pCountCheck" style=" height: 22px; padding: 3px 11px; background: #fafafa;">  
    ...
    <div id="search_pCountCheck" style="float: right;">
        ...
        <div id="menu_pCountCheck" style="width:120px"> 
			<div data-options="name:'pCountCheckId'">热卖汽车类型编号</div> 
...
<div id="pCountCheckEditWindow" class="easyui-window" title="编辑热卖汽车类型" data-options="modal:true,closed:true,
	resizable:true,iconCls:'icon-save',href:'p_count_check/edit'" style="width:65%;height:80%;padding:10px;">
</div>
<div id="pCountCheckAddWindow" class="easyui-window" title="添加热卖汽车类型" data-options="modal:true,closed:true,
	resizable:true,iconCls:'icon-save',href:'p_count_check/add'" style="width:65%;height:80%;padding:10px;">
</div>

<!-- ********************************************************************************** -->
<div id="p2pCountInfo" class="easyui-dialog" title="车辆信息" data-options="modal:true,closed:true,resizable:true,
...
<div id="pCountCheckNoteDialog" class="easyui-dialog" title="热卖汽车类型备注" data-options="modal:true,closed:true,
...
function doSearch_pCountCheck(value,name){ //用户输入用户名,点击搜素,触发此函数  
	if(value == null || value == ''){
		$("#pCountCheckList").datagrid({
	        title:'热卖汽车类型', singleSelect:false, collapsible:true, pagination:true, rownumbers:true, method:'get',
			nowrap:true, toolbar:"toolbar_pCountCheck", url:'p_count_check/list', method:'get',
			loadMsg:'数据加载中......',  fitColumns:true,//允许表格自动缩放,以适应父容器
	        columns : [ [ 
				{field : 'ck', checkbox:true },
				{field : 'pCountCheckId', width : 100, title : '热卖汽车类型编号', align:'center'},
				{field : 'processId', width : 100, align : 'center', title : '车辆编号'},
    		...
    		if(data.msg != null){
    			...
    			$.post("p_count_check/update_note",$("#pCountCheckNoteForm").serialize(), function(data){
    				if(data.status == 200){
    					...
    					$.messager.alert("操作提示", "更新热卖汽车类型备注成功！");
    				}else{
    					$.messager.alert("操作提示", "更新热卖汽车类型备注失败！");
    				}
    			});
    		}
    	});
    ...
    function pCountCheck_edit(){
    	$.get("pCountCheck/edit_judge",'',function(data){
       		if(data.msg != null){
            	...
            	if(ids.length == 0){
            		$.messager.alert('提示','必须选择一个热卖汽车类型才能编辑!');
            		return ;
            	}
            	if(ids.indexOf(',') > 0){
            		$.messager.alert('提示','只能选择一个热卖汽车类型!');
            		return ;
            	}
    ...
    function pCountCheck_delete(){
    	$.get("pCountCheck/delete_judge",'',function(data){
       		if(data.msg != null){
       			...
       			var ids = getPCountSelectionsIds();
               	if(ids.length == 0){
               		$.messager.alert('提示','未选中热卖汽车类型!');
               		return ;
               	}
               	$.messager.confirm('确认','确定删除ID为 '+ids+' 的热卖汽车类型吗？',function(r){
               	    if (r){
               	    	var params = {"ids":ids};
                       	$.post("p_count_check/delete_batch",params, function(data){
                   			if(data.status == 200){
                   				$.messager.alert('提示','删除热卖汽车类型成功!',undefined,function(){
                   					$("#pCountCheckList").datagrid("reload");
                   				});
                   			}
                   		});
