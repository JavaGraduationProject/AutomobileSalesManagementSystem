...
<table  id="pMeasureCheckList" title="人员销售数量" class="easyui-datagrid" data-options="singleSelect:false,
...
<div  id="toolbar_pMeasureCheck" style=" height: 22px; padding: 3px 11px; background: #fafafa;">  
    ...
    <div id="search_pMeasureCheck" style="float: right;">
        ...
        <div id="menu_pMeasureCheck" style="width:120px"> 
			<div data-options="name:'pMeasureCheckId'">人员销售数量编号</div> 
		</div>     
    </div>  
...
<div id="pMeasureCheckEditWindow" class="easyui-window" title="编辑人员销售数量" data-options="modal:true,closed:true,
	resizable:true,iconCls:'icon-save',href:'p_measure_check/edit'" style="width:65%;height:75%;padding:10px;">
</div>
<div id="pMeasureCheckAddWindow" class="easyui-window" title="添加人员销售数量" data-options="modal:true,closed:true,
...
<div id="p2pMeasureInfo" class="easyui-dialog" title="车辆信息" data-options="modal:true,closed:true,resizable:true,
...
<div id="pMeasureCheckNoteDialog" class="easyui-dialog" title="人员销售数量备注" data-options="modal:true,closed:true,
...
function doSearch_pMeasureCheck(value,name){ //用户输入用户名,点击搜素,触发此函数  
	if(value == null || value == ''){
		$("#pMeasureCheckList").datagrid({
	        title:'人员销售数量', singleSelect:false, collapsible:true, pagination:true, rownumbers:true, method:'get',
			nowrap:true, toolbar:"toolbar_pMeasureCheck", url:'p_measure_check/list', method:'get',
			loadMsg:'数据加载中......',  fitColumns:true,//允许表格自动缩放,以适应父容器
	        columns : [ [ 
				{field : 'ck', checkbox:true },
				{field : 'pMeasureCheckId', width : 100, title : '人员销售数量编号', align:'center'},
    ...
    function pMeasureCheck_edit(){
    	$.get("pMeasureCheck/edit_judge",'',function(data){
       		if(data.msg != null){
            	...
            	if(ids.length == 0){
            		$.messager.alert('提示','必须选择一个人员销售数量才能编辑!');
            		return ;
            	}
            	if(ids.indexOf(',') > 0){
            		$.messager.alert('提示','只能选择一个人员销售数量!');
            		return ;
            	}
    ...
    function pMeasureCheck_delete(){
    	$.get("pMeasureCheck/delete_judge",'',function(data){
       		if(data.msg != null){
       			...
       			var ids = getPMeasureSelectionsIds();
               	if(ids.length == 0){
               		$.messager.alert('提示','未选中人员销售数量!');
               		return ;
               	}
               	$.messager.confirm('确认','确定删除ID为 '+ids+' 的人员销售数量吗？',function(r){
               	    if (r){
               	    	var params = {"ids":ids};
                       	$.post("p_measure_check/delete_batch",params, function(data){
                   			if(data.status == 200){
                   				$.messager.alert('提示','删除人员销售数量成功!',undefined,function(){
                   					$("#pMeasureCheckList").datagrid("reload");
                   				});
                   			}
                   		});
