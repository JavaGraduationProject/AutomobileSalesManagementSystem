...
<table class="easyui-datagrid" id="technologyList" title="生产人列表" data-options="singleSelect:false,collapsible:true,
    ...
    <thead>
        <tr>
        	...
        	<th data-options="field:'technologyId',width:100,align:'center'">生产人编号</th>
            <th data-options="field:'technologyName',width:100,align:'center'">生产人名称</th>
            <th data-options="field:'price',width:100,align:'center'">外协价格</th>
            <th data-options="field:'vitalProcessPeriod',width:140,align:'center'">瓶颈车辆工期</th>
            ...
            <th data-options="field:'doubleCapacity',width:140,align:'center'">二倍车辆能力</th>
            <th data-options="field:'overfulfilCapacity',width:160,align:'center'">超负荷车辆能力</th>
        </tr>
    </thead>
</table>
...
<div  id="toolbar_technology" style=" height: 22px; padding: 3px 11px; background: #fafafa;">  
    ...
    <div id="search_technology" style="float: right;">
        ...
        <div id="menu_technology" style="width:120px"> 
			<div data-options="name:'technologyId'">生产人编号</div> 
			<div data-options="name:'technologyName'">生产人名称</div>
		</div>     
    </div>  

</div>

<div id="technologyEditWindow" class="easyui-window" title="编辑生产人" data-options="modal:true,closed:true,
	resizable:true,iconCls:'icon-save',href:'technology/edit'" style="width:40%;height:55%;padding:10px;">
</div>

<div id="technologyAddWindow" class="easyui-window" title="添加生产人" data-options="modal:true,closed:true,
...
function doSearch_technology(value,name){ //用户输入用户名,点击搜素,触发此函数  
	if(value == null || value == ''){
		...
		$("#technologyList").datagrid({
	        title:'生产人列表', singleSelect:false, collapsible:true, pagination:true, rownumbers:true, method:'get',
			nowrap:true, toolbar:"toolbar_technology", url:'technology/list', method:'get', loadMsg:'数据加载中......',
			fitColumns:true,//允许表格自动缩放,以适应父容器
	        columns : [ [ 
				{field : 'ck', checkbox:true },
				{field : 'technologyId', width : 100, align:'center', title : '生产人编号'},
				{field : 'technologyName', width : 100, align : 'center', title : '生产人名称'},
				{field : 'price', width : 100, align : 'center', title : '外协价格'},
				{field : 'vitalProcessPeriod', width : 140, title : '瓶颈车辆工期', align:'center'},
				...
				{field : 'doubleCapacity', width : 140, title : '二倍车辆能力', align:'center'},
				{field : 'overfulfilCapacity', width : 160, title : '超负荷车辆能力', align:'center'},
	        ] ],  
	    });
	}else{
		$("#technologyList").datagrid({  
    ...
    function technology_edit(){
    	$.get("technology/edit_judge",'',function(data){
       		if(data.msg != null){
       	    	...
       	    	if(ids.length == 0){
       	    		$.messager.alert('提示','必须选择一个生产人才能编辑!');
       	    		return ;
       	    	}
       	    	if(ids.indexOf(',') > 0){
       	    		$.messager.alert('提示','只能选择一个生产人!');
       	    		return ;
       	    	}
    ...
    function technology_delete(){
    	$.get("technology/delete_judge",'',function(data){
       		if(data.msg != null){
       			...
       			var ids = getTechnologySelectionsIds();
       	    	if(ids.length == 0){
       	    		$.messager.alert('提示','未选中生产人!');
       	    		return ;
       	    	}
       	    	$.messager.confirm('确认','确定删除ID为 '+ids+' 的生产人吗？',function(r){
       	    	    if (r){
       	    	    	var params = {"ids":ids};
       	            	$.post("technology/delete_batch",params, function(data){
       	        			if(data.status == 200){
       	        				$("#technologyList").datagrid("reload");
       	        				$.messager.alert('提示','删除生产人成功!');
       	        			}else{ 
       	        				$.messager.alert('提示', data.msg);
       	        			}
       	        		});
